...
**E27** G711mu独立注册（g711mu/, 269行）：通过独立PLUGIN_DEFINITION注册，LicenseType=APACHE_V2，不经过ffmpeg_decoder_plugin.cpp三向量体系（G711mu属于OHOS原生实现，非FFmpeg）

```cpp
// g711mu/audio_g711mu_decoder_plugin.cpp L68
PLUGIN_DEFINITION(G711muAudioDecoder, LicenseType::APACHE_V2, RegisterAudioDecoderPlugins,
    UnRegisterAudioDecoderPlugin);
```

**E28** G711a独立注册（g711a/, 283行）：与G711mu并列，G711a-law独立实现，LicenseType=APACHE_V2

```cpp
// g711a/audio_g711a_decoder_plugin.cpp L70
PLUGIN_DEFINITION(G711aAudioDecoder, LicenseType::APACHE_V2, RegisterAudioDecoderPlugins,
    UnRegisterAudioDecoderPlugin);
```

**E29** LBVC独立注册（lbvc/, 274行）：LicenseType=VENDOR（厂商私有codec），独立PLUGIN_DEFINITION，不经过FFmpeg

```cpp
// lbvc/audio_lbvc_decoder_plugin.cpp L62
PLUGIN_DEFINITION(LbvcAudioDecoder, LicenseType::VENDOR, RegisterAudioDecoderPlugins,
    UnRegisterAudioDecoderPlugin);
```

**E30** MP3独立子目录（mp3/, 139行）：虽然MP3在codecVec索引0，但mp3/子目录有独立实现文件，FFmpeg原生MP3解码器通过libmp3decodec接入

```cpp
// mp3/ffmpeg_mp3_decoder_plugin.cpp — MP3独立子目录
constexpr int32_t MIN_CHANNELS = 1;
constexpr int32_t MAX_CHANNELS = 2;
constexpr int32_t SAMPLES = 1152; // MP3每帧样本数
```
## 关键架构特征
...
- Apache V2：RAW、G711mu、G711a解码器（自包含注册）
- VENDOR：LBVC解码器（厂商私有codec）
- 支持codec宏：`SUPPORT_CODEC_TRUEHD`/`SUPPORT_CODEC_DTS`

### 8. 独立注册插件：G711mu/G711a/LBVC（E27-E29）
除ffmpeg_decoder_plugin.cpp主注册体系外，G711mu、G711a（APACHE_V2许可证）和LBVC（VENDOR许可证）三个codec拥有独立的PLUGIN_DEFINITION，各自调用自己的RegisterAudioDecoderPlugins，不经过三向量codecVec/codecInitMap体系。这是OHOS原生codec（不需要FFmpeg解码库）的注册方式，与RAW自包含注册（E17）形成对照。

### 9. MP3独立子目录（E30）
虽然MP3在codecVec索引0（通过ffmpeg_decoder_plugin.cpp注册），但mp3/子目录仍有独立实现文件ffmpeg_mp3_decoder_plugin.cpp。MIN_CHANNELS=1、MAX_CHANNELS=2、SAMPLES=1152，体现了MP3作为最常用音频格式的特殊处理。
...
| E27 | ffmpeg_adapter/audio_decoder/g711mu/audio_g711mu_decoder_plugin.cpp | L68 | G711mu独立PLUGIN_DEFINITION，LicenseType=APACHE_V2，不经过ffmpeg_decoder_plugin.cpp |
| E28 | ffmpeg_adapter/audio_decoder/g711a/audio_g711a_decoder_plugin.cpp | L70 | G711a独立PLUGIN_DEFINITION，LicenseType=APACHE_V2，与G711mu并列原生实现 |
| E29 | ffmpeg_adapter/audio_decoder/lbvc/audio_lbvc_decoder_plugin.cpp | L62 | LBVC独立PLUGIN_DEFINITION，LicenseType=VENDOR，厂商私有codec独立注册 |
| E30 | ffmpeg_adapter/audio_decoder/mp3/ffmpeg_mp3_decoder_plugin.cpp | L139 | MP3独立子目录(139行)，SAMPLES=1152，MIN/MAX_CHANNELS=1-2 |
