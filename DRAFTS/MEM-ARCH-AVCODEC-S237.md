...
| `subSamples[64]` | `DrmSubsample[64]` | 子样本数组 |
...
**Evidence**：
- E5: `native_cencinfo.cpp L103-131` — 参数校验（keyIdLen==16, ivLen==16）+ memcpy_s 两步拷贝
...
**Evidence**：
- E8: `native_cencinfo.cpp L133-153` — encryptedBlockCount→encryptBlocks, skippedBlockCount→skipBlocks, firstEncryptedOffset→firstEncryptOffset, subsampleCount→subSampleNum
...
**Evidence**：
- E11: `native_cencinfo.cpp L170-181` — 参数三元校验（buffer && buffer->buffer_ && buffer->buffer_->meta_）
- E12: `native_cencinfo.cpp L173-177` — `reinterpret_cast` 将 `MetaDrmCencInfo` 转换为 `std::vector<uint8_t>`
- E13: `native_cencinfo.cpp L178` — `buffer->buffer_->meta_->SetData(Tag::DRM_CENC_INFO, std::move(cencInfoVec))`，将 CENC 元数据注入 AVBuffer metadata
...
└─────────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────────┐
│  CodecServer CodecDrmDecrypt (codec_drm_decrypt.cpp L551/L595)        │
...
测试文件：`test/unittest/avcenc_info_test/cenc_info_capi_unit_test.cpp`

| 测试用例 | 描述 |
|---------|------|
| `CencInfo_Create_001` | 创建 CENC info 对象 |
| `CencInfo_Destroy_001` | 销毁 CENC info 对象 |
| `CencInfo_SetAlgorithm_001` | 设置加密算法 |
| `CencInfo_SetKeyIdAndIv_001` | 设置 Key ID 和 IV |
| `CencInfo_SetSubsampleInfo_001` | 设置 Subsample 信息 |
| `CencInfo_SetMode_001` | 设置 CENC 模式 |
| `CencInfo_SetAVBuffer_001` | 将 CENC info 附加到 AVBuffer |
...
| E11 | native_cencinfo.cpp | 170-172 | SetAVBuffer: buffer 三元校验 |
| E12 | native_cencinfo.cpp | 173-177 | reinterpret_cast MetaDrmCencInfo→vector<uint8_t> |
| E13 | native_cencinfo.cpp | 178 | SetData(Tag::DRM_CENC_INFO) 注入 AVBuffer |
...
| 单元测试 | `test/unittest/avcenc_info_test/cenc_info_capi_unit_test.cpp` | ~300+ |
| 模糊测试 | `test/fuzztest/avcencinfo_fuzzer/avcencinfo_fuzzer.cpp` | ~100+ |
